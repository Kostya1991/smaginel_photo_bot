...
export function initCommand() {
    ...
    bot.command("start", async (ctx) => {
        await ctx.reply(
            `
            Привет!\nВ данном боте ты найдешь информацию обо мне, о моих услугах и основных студиях, в которых я работаю🌸, а так же актуальных ценах.\n
            `,
        );
    });
    ...
    bot.command("aboutme", async (ctx) => {
        ...
        await ctx.reply(
            `
            Меня зовут Дарья Лысóва. Я семейный фотограф.\nСоздаю истории о вас настоящих🤍\n\nСнимаю в студии, на природе или у вас дома. Так же снимаю таинство крещения и вернчания.\nЯ помогу с подбором образов, локации, а так же помогу с позингом на фотосессии.\n\nПосле фотосессии я отдаю фото в авторской обработке (цветокоррекция, кадрирование) в течении 10 дней. Первые фото вы получаете в день съемки.\nЗа час съемки вы получаете от 60-100 фото в обработке и 8-10 фото в детальной ретуши.\n\nОбращаю ваше внимание, что после съемки я оставляю за собой право использовать ваши фотографии в своем портфолио, если мы не обговорили иные условия.
    `,
            {parse_mode: "HTML"},
        );
    });
